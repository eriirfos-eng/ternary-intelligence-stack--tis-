...
const GITIGNORE_ENTRIES: [&str; 2] = [".albert/settings.local.json", ".albert/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let albert_dir = cwd.join(".albert");
    artifacts.push(InitArtifact {
        name: ".albert/",
        status: ensure_dir(&albert_dir)?,
    });

    let albert_json = cwd.join(".albert.json");
    artifacts.push(InitArtifact {
        name: ".albert.json",
        status: write_file_if_missing(&albert_json, STARTER_CLAUDE_JSON)?,
    });
    ...
    let albert_md = cwd.join("CLAUDE.md");
    let content = render_init_albert_md(cwd);
    artifacts.push(InitArtifact {
        name: "CLAUDE.md",
        status: write_file_if_missing(&albert_md, &content)?,
    });
...
pub(crate) fn render_init_albert_md(cwd: &Path) -> String {
    ...
    lines.push("- Keep shared defaults in `.albert.json`; reserve `.albert/settings.local.json` for machine-local overrides.".to_string());
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_albert_md};
    use std::fs;
    use std::path::Path;
    ...
    fn temp_dir() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("rusty-albert-init-{nanos}"))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".albert/         created"));
        assert!(rendered.contains(".albert.json     created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("CLAUDE.md        created"));
        assert!(root.join(".albert").is_dir());
        assert!(root.join(".albert.json").is_file());
        assert!(root.join("CLAUDE.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".albert.json")).expect("read albert json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".albert/settings.local.json"));
        assert!(gitignore.contains(".albert/sessions/"));
        let albert_md = fs::read_to_string(root.join("CLAUDE.md")).expect("read albert md");
        assert!(albert_md.contains("Languages: Rust."));
        assert!(albert_md.contains("cargo clippy --workspace --all-targets -- -D warnings"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("CLAUDE.md"), "custom guidance\n").expect("write existing albert md");
        fs::write(root.join(".gitignore"), ".albert/settings.local.json\n")
        ...
        assert!(second_rendered.contains(".albert/         skipped (already exists)"));
        assert!(second_rendered.contains(".albert.json     skipped (already exists)"));
        assert!(second_rendered.contains(".gitignore       skipped (already exists)"));
        assert!(second_rendered.contains("CLAUDE.md        skipped (already exists)"));
        assert_eq!(
            fs::read_to_string(root.join("CLAUDE.md")).expect("read existing albert md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".albert/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".albert/sessions/").count(), 1);
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_albert_md(Path::new(&root));
