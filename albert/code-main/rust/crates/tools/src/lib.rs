...
const DEFAULT_AGENT_MODEL: &str = "albert-opus-4-6";
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "albertcodeguide" | "albertcodeguideagent" | "guide" => String::from("claw-code-guide"),
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".albert").join("settings.local.json"),
    })
}
...
fn config_home_dir() -> Result<PathBuf, String> {
    ...
    Ok(PathBuf::from(home).join(".albert"))
}
...
mod tests {
    ...
    fn agent_fake_runner_can_persist_completion_and_failure() {
        ...
        let completed = execute_agent_with_spawn(
            AgentInput {
                ...
                model: Some("albert-sonnet-4-6".to_string()),
            },
            |job| {
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".albert")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".albert")).expect("cwd dir");
        std::fs::write(
            home.join(".albert").join("settings.json"),
            r#"{"verbose":false}"#,
        )
