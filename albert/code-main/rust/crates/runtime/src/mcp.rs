...
const CLAUDEAI_SERVER_PREFIX: &str = "albert.ai ";
...
pub fn mcp_server_signature(config: &McpServerConfig) -> Option<String> {
    match config {
        ...
        McpServerConfig::AlbertAiProxy(config) => {
            Some(format!("url:{}", unwrap_ccr_proxy_url(&config.url)))
        }
...
pub fn scoped_mcp_config_hash(config: &ScopedMcpServerConfig) -> String {
    let rendered = match &config.config {
        ...
        McpServerConfig::AlbertAiProxy(proxy) => {
            format!("albertai-proxy|{}|{}", proxy.url, proxy.id)
        }
    };
...
mod tests {
    ...
    fn normalizes_server_names_for_mcp_tooling() {
        ...
        assert_eq!(
            normalize_name_for_mcp("albert.ai Example   Server!!"),
            "albert_ai_Example_Server"
        );
        assert_eq!(
            mcp_tool_name("albert.ai Example Server", "weather tool"),
            "mcp__albert_ai_Example_Server__weather_tool"
        );
    }
