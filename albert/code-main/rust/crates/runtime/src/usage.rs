...
mod tests {
    ...
    fn computes_cost_summary_lines() {
        ...
        let lines = usage.summary_lines_for_model("usage", Some("albert-sonnet-4-20250514"));
        assert!(lines[0].contains("estimated_cost=$54.6750"));
        assert!(lines[0].contains("model=albert-sonnet-4-20250514"));
    ...
    fn supports_model_specific_pricing() {
        ...
        let haiku = pricing_for_model("albert-haiku-4-5-20251001").expect("haiku pricing");
        let opus = pricing_for_model("albert-opus-4-6").expect("opus pricing");
