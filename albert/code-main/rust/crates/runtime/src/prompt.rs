...
pub const FRONTIER_MODEL_NAME: &str = "Albert Opus 4.6";
...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            ...
            dir.join(".albert").join("CLAUDE.md"),
            dir.join(".albert").join("instructions.md"),
        ] {
...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if !project_context.instruction_files.is_empty() {
        bullets.push(format!(
            "Albert instruction files discovered: {}.",
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    let mut sections = vec!["# Albert instructions".to_string()];
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".albert")).expect("nested albert dir");
        fs::write(root.join("CLAUDE.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("CLAUDE.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".albert")).expect("apps albert dir");
        fs::write(root.join("apps").join("CLAUDE.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".albert").join("instructions.md"),
            "apps dot albert instructions",
        )
        .expect("write apps dot albert instructions");
        fs::write(nested.join(".albert").join("CLAUDE.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".albert").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.albert/CLAUDE.md")),
            "CLAUDE.md"
        );
    }
    ...
    #[test]
    fn load_system_prompt_reads_albert_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".albert")).expect("albert dir");
        fs::write(root.join("CLAUDE.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".albert").join("settings.json"),
        ...
        let original_albert_home = std::env::var("CLAUDE_CONFIG_HOME").ok();
...
",
        ...
        if let Some(value) = original_albert_home {
    ...
    #[test]
    fn renders_albert_code_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".albert")).expect("albert dir");
        ...
        assert!(prompt.contains("# Albert instructions"));
        assert!(prompt.contains("Project rules"));
        assert!(prompt.contains("permissionMode"));
    ...
    #[test]
    fn discovers_dot_albert_instructions_markdown() {
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(nested.join(".albert")).expect("nested albert dir");
        fs::write(
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".albert/instructions.md")));
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        ...
        assert!(rendered.contains("# Albert instructions"));
