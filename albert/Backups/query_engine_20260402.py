...
class QueryEnginePort:
    ...
    def submit_message(
        ...
        if self.config.lite_mode:
            # Lite mode: don't inject a competing system prompt — let the model's
            # built-in nano.mf SYSTEM + MESSAGE few-shot pairs do the work.
            # Injecting our own system message here would override the few-shot examples.
            full_history = list(self.messages)
        else:
        ...
        for iteration in range(self.config.max_turns):
            try:
                ...
                if "support tools" in str(e).lower():
                    # Fallback to text-only and rely on neurosymbolic parsing
                    # Add a hint to the system prompt for text-based tool calling
