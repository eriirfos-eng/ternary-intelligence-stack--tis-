...
def delete_session(sid):
    conn.cursor().execute("DELETE FROM sessions WHERE id=?", (sid,))
    conn.cursor().execute("DELETE FROM messages WHERE session_id=?", (sid,))
    conn.commit()
...
def search_library(query):
    ...
    if len(query) < 15:
        quick = [f"FILE: {f[0]}\n{f[1][:1000]}" for f in files if query.lower() in f[1].lower()]
        if quick:
            return "\n\n---\n\n".join(quick[:2])
    results = _get_tfidf_results(tuple(f[1] for f in files), query)
    res     = [f"FILE: {files[i][0]}\n{files[i][1][:1500]}" for i, _ in results]
...
TOOLS_DEF = [
    ...
    {'type': 'function', 'function': {
        'name': 'search_library',
        'description': 'Search uploaded research files in the local library.',
...
def _extract_tool_calls_from_text(content: str):
    ...
    if sql_match:
        ...
        fixed    = _fix_table_aliases(raw_sql).strip()
        safe_sql = fixed.replace('"', '\\"')
        return [{'function': {'name': 'execute_bash', 'arguments': {'command': f'sqlite3 {DB_PATH} "{safe_sql}"'}}}]
    ...
    for iteration in range(8):
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for tc in m['tool_calls']:
                ...
                status.markdown(
                    f"<div style='font-size:0.8rem;color:#888;padding-left:1rem'>"
                    f"{random.choice(thinking_labels)}… ⚡ <code>{fn}</code>"
...
_TOOL_KEYWORDS = (
    ...
    "execute", "install", "check", "health", "cpu", "ram", "disk",
    "list", "show me", "what is", "fetch", "read", "write", "create",
    "delete", "find", "open", "kill", "start", "stop",
)
...
.thinking-dot{display:inline-block;width:8px;height:8px;background:var(--accent);border-radius:50%;margin-left:6px;vertical-align:middle;animation:pulse 1.2s infinite ease-in-out;box-shadow:0 0 5px var(--accent);}
...
[data-testid="stChatMessage"]{background:#1c212e!important;border:1px solid var(--border);border-radius:14px;margin-bottom:1rem;position:relative;padding:.8rem 1.2rem;box-shadow:0 4px 12px rgba(0,0,0,.15);max-width:85%;width:fit-content;}
...
.msg-actions{display:none;position:absolute;top:12px;right:12px;gap:6px;align-items:center;z-index:10;}
[data-testid="stChatMessage"]:hover .msg-actions{display:flex;}
.msg-btn{background:#262c3d;border:1px solid var(--border);color:var(--text);border-radius:4px;padding:2px 8px;font-size:.72rem;cursor:pointer;font-family:'JetBrains Mono',monospace;}
...
with st.sidebar:
    ...
    with st.expander("⚙️ ENGINE", expanded=False):
        ...
        st.slider("CPU Threads",       1, num_cores, 4,    key="t_threads")
        st.slider("GPU Layers (VRAM)", 0, 35,        0,    key="t_gpu", help="10-15 for 2 GB VRAM.")
...
if dmsgs and dmsgs[-1]["role"] == "user":
    ...
    with st.chat_message("assistant"):
        ...
        opts   = {
            ...
            "num_thread":  st.session_state.get("t_threads", 4),
            "num_gpu":     st.session_state.get("t_gpu",  0),
        }
        ...
        placeholder.markdown(f"{marker}<span class='thinking-dot'></span>", unsafe_allow_html=True)
        ...
        try:
            if use_agent:
                ...
                _status.empty()
                placeholder.markdown(f"{marker}{out}", unsafe_allow_html=True)
                ...
                for chunk in res:
                    ...
                    placeholder.markdown(
                        f"{marker}{out}<span class='thinking-dot'></span>"
                        f"<div style='font-size:.65rem;color:#555'>TPS: {tps:.1f}</div>",
                        unsafe_allow_html=True
                    )
                placeholder.markdown(f"{marker}{out}", unsafe_allow_html=True)
...
if user_in:
    ...
    if _row and _row[0] == "New Node":
        ThreadPoolExecutor().submit(auto_title_session, st.session_state.sid, user_in, get_local_models()[0])
    st.rerun()
